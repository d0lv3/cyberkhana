...
const rankIcon = (rank: number) => {
  ...
  return <span className="text-sm font-semibold text-muted">{rank}</span>;
};
...
const podiumFireParticles = [
  { left: '16%', bottom: 24, delay: 0.0, duration: 1.2, size: 'w-1.5 h-1.5', color: 'from-[#f59e0b] via-[#fde68a] to-brand-neon' },
  { left: '28%', bottom: 18, delay: 0.15, duration: 1.35, size: 'w-2 h-2', color: 'from-brand-neon via-[#fde68a] to-[#f59e0b]' },
  { left: '40%', bottom: 22, delay: 0.3, duration: 1.15, size: 'w-1 h-1', color: 'from-[#f59e0b] via-[#facc15] to-brand-neon' },
  { left: '56%', bottom: 20, delay: 0.45, duration: 1.25, size: 'w-2 h-2', color: 'from-[#facc15] via-brand-neon to-[#f59e0b]' },
  { left: '70%', bottom: 26, delay: 0.6, duration: 1.4, size: 'w-1.5 h-1.5', color: 'from-[#fbbf24] via-[#f59e0b] to-brand-neon' },
  { left: '82%', bottom: 18, delay: 0.75, duration: 1.1, size: 'w-1 h-1', color: 'from-brand-neon via-[#fde68a] to-[#f59e0b]' },
];

const podiumInnerParticles = [
  { left: '18%', top: '62%', delay: 0.1, duration: 1.2, size: 'w-1 h-1', color: 'from-[#f59e0b] to-brand-neon' },
  { left: '33%', top: '58%', delay: 0.25, duration: 1.35, size: 'w-1.5 h-1.5', color: 'from-brand-neon to-[#f59e0b]' },
  { left: '48%', top: '66%', delay: 0.4, duration: 1.15, size: 'w-1 h-1', color: 'from-[#facc15] to-brand-neon' },
  { left: '62%', top: '60%', delay: 0.55, duration: 1.3, size: 'w-1.5 h-1.5', color: 'from-brand-neon to-[#facc15]' },
  { left: '77%', top: '64%', delay: 0.7, duration: 1.2, size: 'w-1 h-1', color: 'from-[#fbbf24] to-brand-neon' },
];
...
const PodiumCard: React.FC<{
  ...
  return (
    <div className="relative pt-4">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 top-0 z-30 px-3 py-1 rounded bg-amber text-surface text-xs font-black shadow-sm">
        {rank}
      </div>
      ...
      >
        ...
        <div className="absolute inset-[1.5px] overflow-hidden bg-surface" style={{ clipPath: shapePath }}>
          {rank === 1 && (
          ...
          <div className="relative z-20 h-full flex flex-col items-center justify-center px-4 text-center">
            <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full border ${accent.border} bg-panel flex items-center justify-center overflow-hidden`}>
              {entry.avatarUrl ? (
                <img src={entry.avatarUrl} alt={entry.player} className="w-full h-full object-cover" />
              ) : (
                <span className="text-2xl font-black text-brand-neon">{entry.player.charAt(0).toUpperCase()}</span>
              )}
            </div>
            ...
            {entry.playerTag ? (
              <span className="mt-2 inline-flex max-w-full items-center rounded-full border border-[#3a4864] bg-panel px-2.5 py-1 text-[11px] font-semibold text-muted">
                <span className="truncate" title={entry.playerTag}>{entry.playerTag}</span>
              </span>
            ) : null}
            <p className="text-sm text-muted mt-1">{points} pts</p>
          </div>
        </div>
      </div>
    </div>
...
const UnifiedLeaderboard: React.FC<UnifiedLeaderboardProps> = ({
  ...
  if (loading) {
    return <div className="text-muted text-sm">Loading leaderboard...</div>;
  }
  ...
  return (
    <div className="relative text-fg-soft">
        <div className="mb-8">
          <div className="inline-flex items-center rounded border border-edge-soft bg-surface px-4 py-1 text-xs text-muted mb-4">
            Player Leaderboard
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-fg">{title}</h1>
        ...
        <section className="grid lg:grid-cols-[minmax(0,1fr)_260px] gap-5">
            ...
            <div className="flex flex-wrap gap-3">
              <div className="relative flex-1 min-w-[220px]">
                ...
                <input
                  ...
                  className="w-full bg-[#141c2b] border border-edge-soft rounded px-9 py-2 text-sm text-[#e2e8f6] placeholder:text-faint focus:outline-none focus:border-[#3a4864]"
                />
              </div>
            </div>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0 flex items-center gap-2">
                              ...
                              {isCurrent ? <span className="text-[11px] text-brand-neon">YOU</span> : null}
                            </div>
                          </div>
                        </td>
          ...
          <aside className="space-y-4">
            <div className="rounded-xl border border-edge bg-surface p-4">
              ...
              <p className="text-xs text-muted-alt mt-1">FLAGS PWNED</p>
              <div className="h-2 rounded bg-[#0f1624] border border-edge-soft mt-3 overflow-hidden">
                <div className="h-full bg-brand-neon" style={{ width: `${progressPct}%` }} />
              </div>
            </div>

            <div className="rounded-xl border border-edge bg-surface p-4 text-center">
              <p className="text-xs tracking-wider text-dim mb-3">YOUR RANK</p>

              <div className="mx-auto w-28 h-28 border border-edge-light bg-panel flex items-center justify-center"
                style={{ clipPath: 'polygon(25% 6%,75% 6%,94% 25%,94% 75%,75% 94%,25% 94%,6% 75%,6% 25%)' }}>
                <Shield className="w-9 h-9 text-brand-neon" />
              </div>

              <p className="mt-3 text-2xl font-black text-fg">
                {currentPlayerRank ? `#${currentPlayerRank}` : 'UNRANKED'}
              </p>
              <p className="text-xs text-muted-alt mt-1">{currentPlayer ? `${currentPlayer.points} pts` : ''}</p>
            </div>
          </aside>
        </section>
    </div>
