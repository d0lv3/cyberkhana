...
const EventCard: React.FC<{ event: any; onChange: () => void }> = ({ event, onChange }) => {
  ...
  return <article className="rounded-2xl border border-indigo-400/30 bg-gradient-to-br from-indigo-500/10 to-panel p-5 flex flex-col gap-4">
    ...
    <div className="flex flex-wrap gap-3 mt-auto">
      {(event.registered || event.canManage) && <Link to={event.canManage ? `/admin/competitions/${event._id}/monitor` : `/competition/${event._id}`} className="rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white">{event.canManage ? 'Manage event' : 'Registered · Open event'}</Link>}
